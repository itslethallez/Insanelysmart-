...
describe("visit assets on Vercel", () => {
  ...
  it("keeps an id-wallet tree in case Vercel Root Directory is still set to that", () => {
    ...
    assert.equal(
      readFileSync(path.join(nested, "src/server.ts"), "utf8"),
      readFileSync(path.join(repoRoot, "src/server.ts"), "utf8"),
    );
    assert.ok(existsSync(path.join(nested, "api/index.ts")));
    assert.ok(existsSync(path.join(nested, "id-wallet/package.json")));
  });
});
